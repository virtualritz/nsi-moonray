...
| **An `instances` node is instanced, not expanded** | Open | `research.md` F9 — MoonRay's `RdlInstancerGeometry` (`references`, `xform_list`, `ref_indices`, `instance_level` to 4) and upstream's `instance_sources` / `instance_transforms` / `Instance` meet almost one to one | None. `flush.rs` handles `ResolveError::Instanced` by reporting it, and never asks the question that succeeds — so an `instances` node contributes nothing and a crowd renders as one prototype at the origin | Emit the instancer; assert the *object count* as well as the image, since a flattened scene renders identically and is the failure this exists to prevent. `T6.1`, `T6.5` |
