...
| An unmapped shader fails loudly | **Partial** | `flush.rs` -- every ɴsɪ shader is unmapped, since MoonRay has no OSL (`research.md` F6); the `Layer` row is written with `undef()` in the material column | `flush::tests::a_bound_shader_is_reported_and_the_scene_still_emits` -- the scene still emits and the limitation is reported | A substitute material rather than none, so a shaded scene does not silently render in MoonRay's default grey. `T1.3` |
