...
| The camera's field of view | **Covered** | `PerspectiveCamera::computeProjectionMatrix` -- the vertical half-angle is `atan(halfFilmWidth * height / width / focal)`; ɴsɪ's `fov` is vertical, measured with `tools/probe/framing.nsi` against 3Delight (`research.md` F11) | `flush::tests::fov_becomes_a_focal_length` for the arithmetic; `inprocess::the_frame_matches_3delights_framing` renders the probe through MoonRay and lands on 3Delight's rectangle to within a pixel | -- |
...
| **One shutter for the whole scene** | **Covered** | MoonRay evaluates every `blur(a, b)` at two *global* timesteps (`SceneVariables::motion_steps`), so per-object intervals render one object's motion during another's shutter | `flush::tests::two_objects_share_one_shutter` — one moving over `[0,1]` and one over `[2,4]`, both sampled at the union's ends with the later one held rather than extrapolated; `a_shutter_range_beats_the_union`; `deformation_is_resampled_onto_the_shutter` | -- |
...
| Deformation motion blur | **Covered** | `flush.rs` -- `attribute_samples(handle, "P")` into `vertex_list_0` and `vertex_list_1`. Two *attributes*, not a `blur()` pair: that form is for scalars and matrices | `flush::tests::a_deforming_mesh_gets_two_vertex_lists`, plus the static, over-sampled and changing-vertex-count cases; `inprocess::a_deforming_mesh_renders_blurred` counts the partially covered columns a smear leaves | -- |
...
| Subdivision reaches the limit surface | **Covered** | `flush.rs` -- `subdivision.scheme` on a `mesh` sets `is_subd`, with creases and corners carried across | `flush::tests::subdivision_scheme_on_a_mesh_makes_it_a_subdivision_surface` and `creases_and_corners_cross`; a `polyhedron-ops` polyhedron rendered at four crease hardnesses visibly rounds | `inprocess::subdivision_reaches_the_limit_surface` renders a cube both ways and compares coverage — the limit surface rounds inward. View-adaptive tessellation is separate and unasserted. |
...
| An unmapped shader fails loudly | **Partial** | `flush.rs` -- MoonRay runs no OSL (`research.md` F6), so every ɴsɪ shader becomes a `UsdPreviewSurface` and the `Layer` row points at that. What it carries comes from `PARAMETERS`, keyed on `shaderfilename` and read off 3Delight's shipped `.oso` files rather than guessed (`research.md` F11) | `flush::tests::a_bound_shader_becomes_the_default_surface` -- the substitution happens, the scene emits, and the limitation is reported; `a_known_shader_is_carried_by_its_own_parameter_names`, `an_unknown_shader_falls_back_to_exact_names`, `a_shader_path_still_names_its_shader` | The shading itself, which needs OSL. `TN.2` |
