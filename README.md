...
| Mesh lights | A `MeshLight`'s geometry also carrying a material | `RenderContext::createMeshLightLayer` warns and skips a light whose geometry is in the main render `Layer`, so it can't wear a material -- kept out of the `Layer`, forced visible in camera instead. It is always kept *in* the `GeometrySet`, unconditionally, which is not a choice this backend has room to get wrong: MoonRay itself segfaults at render prep if such geometry is in neither and later gets a `map_shader` -- a real MoonRay bug, not this backend's, written up for MoonRay's own issue tracker at `upstream/moonray-meshlight-map-shader-segfault.md`. |
...
| Curve basis | `hobby` | `linear`, `b-spline` and `catmull-rom` all cross exactly -- `catmull-rom` by a basis conversion to `bezier` ([`uniform-cubic-splines`](https://crates.io/crates/uniform-cubic-splines)), and `extrapolate` by adding the phantom end point a `b-spline`/`catmull-rom` cage needs to reach its own end vertices. `hobby` solves a global curvature-minimising system, not a fixed per-segment basis, so there is no exact conversion -- approximated as `catmull-rom` (a smooth interpolating spline through the same points) rather than dropped to `linear`, which would discard its curvature entirely. |
